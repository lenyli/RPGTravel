...
游戏风格为选填，初始留空，没有预设内容或默认游戏参考；不填时按目的地和其他已填需求生成故事，`adventure.gameStyle` 使用空列表。旧草稿中原有的固定游戏预设会清空，其他自填内容保留。
...
导入会先提取完整 JSON：允许说明文字、混合围栏、缺少外层标记和内容相同的重复包；确有不同故事时直接弹出选择框。章节 `subtitle` 合入简介，缺少区域沿用已有章节标题，缺少简介留空；完整但被平铺或放入 `meta/trip/metadata` 的冒险概要会归位。兼容过程保留原始回答，缺失的核心剧情、任务与引用不会被猜补，结构错误明确指出缺失字段或实际类型。
...
AI 文本使用普通 React 文本节点和 `white-space: pre-wrap` 展示，不执行 HTML。来源网址按原文导入、保存、展示和导出，不做网址格式、协议或长度校验；整包容量上限仍适用。普通或 Markdown 包裹的 http/https 来源可以点击，其余内容保留为文字，不阻挡故事导入。外部链接使用 `noopener noreferrer`。导航地址与安全提醒不受剧情锁影响。
