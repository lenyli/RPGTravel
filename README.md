...
整个应用为纯前端单页应用（SPA），旅行进度与照片存储在用户本地浏览器，无需登录注册。生成故事时，可以复制提示词交给自己的 AI，也可以在页面里保存自己的 DeepSeek API Key，由浏览器直接请求。本项目没有账号，也不保存旅行数据或 API Key。线上页面尚未用真实 Key 完成生成验收。
...
- 🔒 **纯本地隐私存储**：进度、日志、照片保存在浏览器的 IndexedDB 中。可选的 DeepSeek Key 也只留在当前浏览器，不写入故事备份。
- ✍️ **短提示生成**：默认复制短提示词。打开 AI 后使用固定的 `deepseek-flash`，不提供其他模型。
