...
4. App 只显示文本，不提供额外的图片、背包或档案子页面。当前任务行动需要的背景与剧情物品文字必须完整写在本任务 story.scene 中，不能要求先取得别处线索；不得要求打开没有提供的附件、图片或按钮。
...
9. 参考我填写的游戏在氛围、探索与叙事上的特点，不照搬原游戏角色或完整剧情。
...
来源写普通 URL，不输出聊天产品特有的引用标记、脚注占位符或伪造的网页。
...
- 所有字段必须出现；未知时区和坐标用 null，空人物与来源用 []。仅 subtitle、transportNote、accessNote、safetyNote 可为空文本，其他文字不允许纯空白。
...
- 来源 URL 只用有效 http/https 页面；checkedOn 必须是实际查阅日期，不能使用提示生成日期冒充。无法实际查阅时 sources=[] 并说明核验限制。
- 用户已填写的住宿、预约、体力及不去地点是硬约束。可选输入为空时使用合理默认（游戏氛围默认参考塞尔达传说 / 巫师 3），不反问，也不忽略已给限制。
